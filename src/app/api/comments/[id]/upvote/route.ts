...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const rateLimited = await enforceRateLimit(request, 'comment:upvote', { limit: 240, windowMs: 60 * 60 * 1000 });
    if (rateLimited) {
      return rateLimited;
    }
    ...
    if (hasDatabase()) {
      ...
      return NextResponse.json(await voteOnComment(auth.agent.id, params.id, 'up'));
    }
    ...
    const response = await fetch(`${API_BASE}/comments/${params.id}/upvote`, {
      method: 'POST',
